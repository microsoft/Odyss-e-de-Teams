...
import { IModule, INiveau, IChoixModuleNiveauState, IChoixModuleNiveauProps } from "src/models/Question";
...
class ChoixModuleNiveau extends Component<IChoixModuleNiveauProps, IChoixModuleNiveauState> {
    ...
    render() {
        return (
            <div className={"main-choix-module mb-0"}>
                <h2 className={"d-none d-md-block color-primary-light mb-2"}>Mission xx : </h2>
                <h2 className={"d-block d-md-none mb-2"}>Mission xx : </h2>
                <p className={"d-none d-md-block mb-2"}>Ta première mission est arrivée : ici commence ton exploration galactique à travers les usages collaboratifs de Teams</p>
                <h4 className={"mt-md-3 mb-2"}>Sélectionne un module et sa difficulté puis prépare-toi au décollage !</h4>
                <FadeInUpModule>
                    <div className={"d-flex flex-column flex-md-row mt-4"}>
                        {
                            this.state.listModule?.map((item: IModule) => {
                                return (
                                    <div key={item.id_module} className={`mx-1 module module${item.id_module}`}>
                                        <div className={"content d-flex flex-column justify-content-between h-100 p-2"}>
                                            <p className={"text-center mb-0"}>
                                                <img
                                                    src={process.env.PUBLIC_URL + "/images/question/module/" + item.image}
                                                    alt={`Illustration module ${item.nom}`}
                                                    className={"illustration-module"}
                                                />
                                            </p>
                                            <h4 className={"d-flex align-items-center justify-content-center text-center color-primary-light h-100 mb-2"}>{item.nom}</h4>
