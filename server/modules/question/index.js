...
const register = async (server, options) => {
  ...
  server.route({
    ...
    handler: function (request, h) {
      ...
      return db.sequelize
        .query(
          "SELECT DISTINCT a.id_module, TRIM(b.nom) AS nom, a.image FROM public.t_module a INNER JOIN public.t_libelle_i18n b ON a.id_module=b.id_table AND TRIM(b.code)='MODULE' AND TRIM(b.lang)=:lang WHERE a.actif ORDER BY TRIM(b.nom)",
          {
            replacements: replacements,
            type: QueryTypes.SELECT,
          }
  ...
  server.route({
    ...
    handler: function (request, h) {
      ...
      return db.sequelize
        .query(
          "SELECT a.id_niveau, TRIM(b.nom) AS nom FROM public.t_niveau a INNER JOIN public.t_libelle_i18n b ON a.id_niveau=b.id_table AND TRIM(b.code)='NIVEAU' AND TRIM(b.lang)=:lang WHERE a.actif ORDER BY a.ordre",
          {
            replacements: replacements,
            type: QueryTypes.SELECT,
          }
