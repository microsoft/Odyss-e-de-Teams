    ...
    server.route({
        ...
        handler: async function (request, h) {
            ...
            let main_query = `
                ...
                SELECT DISTINCT a.id_agenda, TRIM(a.nom) AS nom, a.date_agenda, CASE WHEN b.id_agenda_done IS NOT NULL THEN true ELSE false END AS done 
                FROM public.t_agenda a
                    LEFT JOIN wdone b ON a.id_agenda=b.id_agenda 
                WHERE true`;
            if (params.mode && params.mode === 'nb_restant') {
                main_query = "SELECT COUNT(*) AS nb FROM (" + main_query + " AND b.id_agenda_done IS NULL)s0";
                oneResult = true;
            }
